...
def fetch_yfinance(symbol: str):
    ...
    if interest_income is not None and not (isinstance(interest_income, float) and math.isnan(interest_income)) and interest_income > 0 and \
       total_revenue is not None and not (isinstance(total_revenue, float) and math.isnan(total_revenue)) and total_revenue > 0:
        purification_ratio = (interest_income / total_revenue) * 100
        ...
        try:
            ...
            if interest_income is not None and not (isinstance(interest_income, float) and math.isnan(interest_income)) and \
               total_revenue is not None and not (isinstance(total_revenue, float) and math.isnan(total_revenue)):
                ...
                # --- هذا هو المنطق النهائي والمصحح ---
                if interest_income <= 0:
                    # إذا كان صافي دخل الفائدة سالباً أو صفراً، فالشرط مُجتاز
                    rev_check_result = True
                elif total_revenue > 0:
                    # إذا كان الدخل موجب، نحسب النسبة
                    # نستخدم interest_income مباشرة لأنه موجب
                    rev_check_result = (interest_income / total_revenue) < 0.05
                else: # إيرادات صفر أو أقل، ودخل الفوائد موجب
...
def _build_financial_report_text(lang, company, sym, metrics_data, report_date, interest_income, total_revenue, market_cap_update_time=None):
    ...
    def get_formatted_value(key, value, lang):
        ...
        if key == "interest_income":
            # إذا كانت قيمة الدخل سالبة أو غير موجودة، نعرضها كـ 0
