...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # يتم إضافة المستخدم لقاعدة البيانات فقط إذا لم يكن موجوداً
    db.add_user_if_not_exists(user.id, user.first_name, user.username)
    
    # يتم البحث عن اللغة المحفوظة لهذا المستخدم
    lang = db.get_user_setting(user.id, 'language')

    # الشرط: إذا لم تكن هناك لغة محفوظة (يعني مستخدم جديد)
    if not lang:
        # أظهر له أزرار اختيار اللغة
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # --- *** هذا هو الجزء الذي تم تعديله بناءً على طلبك *** ---
        ...
        if not is_compliant_somewhere:
             # إذا كان غير متوافق مع الكل أو نشاطه محرم، نعرض ❌
             pur_text = "❌"
        elif pur_val is not None and not math.isnan(pur_val):
            # إذا كان متوافقاً مع واحد على الأقل، ولدينا نسبة، نعرضها
            if pur_val > 0:
                pur_text = f"{pur_val:.2f}%{MESSAGES[lang]['purification_mixed_text']}"
            else: # pur_val is 0.0 or less
